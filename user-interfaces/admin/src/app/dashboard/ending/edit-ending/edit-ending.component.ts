...
@Component({
  selector: 'app-edit-ending',
  templateUrl: './edit-ending.component.html',
  styleUrls: ['./edit-ending.component.scss']
})
export class EditEndingComponent {
  ...
  completion!: ICompletionAttributes;
  ...
  constructor(
    ...
    this.subscription = this.route.params.subscribe((params) => {
      this.completionId = this._decryptService.decrypt(params['id']);
    });

    this.getCompletionById(this.completionId);

  }
  setUpCompletionForm(completion: ICompletionAttributes): void {
  ...
  updateCompletion(formValue: FormGroup, step: MatStepper) {
    ...
    this.subscription = this._completionService.updateCompletion(completion, this.completion.id).subscribe(
      (completion) => {
        this.isRequestLoading = false;
        this._notificationService.showSuccessNotification('Updated Successfully!', 'Completion has been updated.');
  ...
  getCompletionById(id: string | number): void {
    this.isLoading = true;
    this.subscription = this._completionService.getCompletionById(id).subscribe(
      (completion) => {
        this.isLoading = false;
        this.completion = completion.data;
        console.log('Completion Details ', this.completion);
        this.setUpCompletionForm(this.completion);
  ...
  deleteCompletionDocument(event: number | string): void {
    ...
    this.subscription = this._documentService.deleteDocument(event).subscribe(
      (document) => {
        this.isRequestLoading = false;
        this.getAllDocuments({ 'projectID': this.completion.project_id, 'section': 7 });
